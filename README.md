...
> 현재 버전은 초기 개발 단계의 `0.1.4`이며 Windows x64와 한국어 중심 UI를 대상으로 합니다.

## 화면과 학습 흐름

### 1. 첫 실행 환경을 따로 설정합니다

처음 실행하면 학습 화면보다 먼저 데이터 저장 위치, 라이트·다크 모드, Codex·Claude 실행 경로와 개인 API 연결을 확인합니다. 로그인된 Codex CLI가 확인되면 API 키 없이 시작할 수 있고, 그렇지 않으면 OpenAI·Anthropic·DeepSeek 중 하나의 연결 확인을 마쳐야 과목 등록으로 넘어갑니다. 설정은 나중에 다시 열 수 있습니다.
...
- 라이트·다크 모드
- 첫 실행 전용 데이터·화면·AI 연결 설정
- 구조화 응답 실패의 1회 자동 재시도, 진행 안내와 요청 취소
- 수동으로 닫는 오류 알림, 최근 50건 오류 기록과 요청 ID
- 과목 입력 및 학습 지시 입력창의 Tab 추천 문장 완성·반복 순환
- 호출별 공급자, 토큰, 비용 사용 이력
...
2. ZIP을 쓰기 가능한 새 폴더에 압축 해제합니다. 예: `D:\StudyQuest`
3. 압축을 푼 폴더의 `StudyQuest.cmd`를 실행합니다.
4. 첫 실행 설정에서 저장 위치와 사용할 AI 연결을 확인합니다.

배포 ZIP은 `StudyQuest.exe`가 압축 루트 바로 아래에 오도록 구성하고 내부 Codex 실행 파일 경로도 줄여 Windows 경로 길이 문제를 완화했습니다. 기존 릴리스 위에 덮어쓰기보다 새 폴더에 압축을 푸는 것을 권장합니다.
...
원시 Electron 패키지는 `portable/StudyQuest-win32-x64/`에 생성됩니다. 실제 배포와 같은 평평한 폴더를 만들고 필수 파일·라이선스·빈 데이터 폴더·ZIP 내부 경로 길이를 함께 검증하려면 다음을 실행합니다.

```powershell
npm.cmd run package:release
```

검증된 배포 폴더는 `release/StudyQuest/`에 생성되며 `StudyQuest.exe`와 `StudyQuest.cmd`가 같은 루트에 놓입니다.
...
구조화 JSON이 비거나 스키마에 맞지 않으면 전체 제한 시간 안에서 같은 공급자에 한 번 자동 재시도합니다. 자동 라우팅이면 복구되지 않은 뒤 연결된 다음 공급자로 넘어갈 수 있습니다. 대기 중에는 진행 문구와 취소 버튼을 표시하고, 실패해도 작성 중이던 입력을 지우지 않습니다. 오류 알림은 직접 닫기 전까지 유지되며 `AI와 설정`에 최근 50건의 사용자용 오류와 요청 ID를 보관합니다. 프롬프트와 API 키는 오류 기록에 넣지 않습니다.

저장된 `studyquest.json`을 읽거나 이전 상태를 변환하지 못하면 자동 저장을 중단해 원본 덮어쓰기를 막습니다. 복구 화면에서 사용자가 직접 승인한 경우에만 원본을 고유한 `.recovery-<timestamp>-<uuid>.bak` 파일로 먼저 이동한 뒤 빈 학습 상태로 시작합니다.
...
- 실행 파일 버전뿐 아니라 `codex login status`로 로그인 상태까지 확인하며, 인증되지 않았으면 첫 실행 설정에서 `codex login`을 안내합니다.
...
- 저장만 한 키는 라우팅 후보가 되지 않으며 실제 연결 확인에 성공해 `verifiedAt`이 기록되어야 사용 가능합니다. 인증·권한·잔액·모델·응답 형식의 하드 실패가 발생하면 암호문은 보존하되 검증 상태를 해제해 다시 확인하도록 안내합니다.
- DeepSeek 연결 확인은 공식 [모델 목록 API](https://api-docs.deepseek.com/api/list-models)에 선택한 모델이 실제로 있는지 검사합니다. [JSON 출력 모드](https://api-docs.deepseek.com/guides/json_mode/)가 빈 응답을 돌려주는 일시적 상황에는 같은 요청을 한 번 재시도합니다.
...
- 공급자 로그인이나 API 권한은 앱 실행 뒤 변경될 수 있으므로 AI 호출 직전에 연결 상태를 다시 확인합니다.
